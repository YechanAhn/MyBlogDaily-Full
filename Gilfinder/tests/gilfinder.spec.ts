...
test.describe('페이지 로딩', () => {
  ...
  test('페이지 타이틀이 올바르다', async ({ page }) => {
    await page.goto('/');
    await expect(page).toHaveTitle(/길위의발견|GilFinder/);
  });
  ...
  test('OG 태그가 설정되어 있다', async ({ page }) => {
    ...
    expect(ogTitle).toContain('길위의발견');
  });
...
test.describe('카카오맵 로딩', () => {
  ...
  test('카카오맵 SDK mock 주입 시 지도가 초기화된다', async ({ page }) => {
    ...
    // mock된 SDK로 맵이 초기화되면 로딩 오버레이가 사라져야 함
    const loadingText = page.getByText('지도를 불러오는 중...');
    const isHidden = await loadingText.isHidden().catch(() => true);
...
// ─────────────────────────────────────────────
test.describe('검색 기능', () => {
  test('검색바가 홈 화면에 표시된다', async ({ page }) => {
    await page.goto('/');
    const searchInput = page.locator('input[placeholder="어디로 갈까요?"]');
  ...
  test('검색바 클릭 시 검색 뷰로 전환된다', async ({ page }) => {
    ...
    // search 뷰에서는 "도착지를 검색하세요" placeholder가 보여야 함
  ...
  test('검색 뷰에서 텍스트를 입력할 수 있다', async ({ page }) => {
    await page.goto('/');
    // 먼저 검색 뷰로 전환
    const homeInput = page.locator('input[placeholder="어디로 갈까요?"]');
  ...
  test('검색어 입력 시 API가 호출된다', async ({ page }) => {
    ...
    // debounce 후 API 호출이 발생하는지 확인
    const searchRequest = await searchRequestPromise;
    // API 요청이 발생했으면 성공 (응답이 403이어도 코드가 올바르게 동작하는 것)
    if (searchRequest) {
  ...
  test('검색 뒤로가기 버튼이 동작한다', async ({ page }) => {
    ...
    if (await backButton.count() > 0) {
      ...
      // 홈 뷰로 복귀
      const returnedInput = page.locator('input[placeholder="어디로 갈까요?"]');
...
test.describe('UI 컴포넌트 렌더링', () => {
  ...
  test('검색바 아이콘(돋보기)이 표시된다', async ({ page }) => {
    await page.goto('/');
    const searchIcon = page.locator('svg circle[cx="11"][cy="11"][r="8"]');
    await expect(searchIcon.first()).toBeVisible();
  });

  test('result 페이지가 로딩된다', async ({ page }) => {
    const response = await page.goto('/result');
    expect(response?.status()).toBe(200);
    // Suspense fallback의 로딩 텍스트가 보이거나, 에러 상태가 보여야 함
  ...
  test('홈 화면에 검색바 wrapper가 shadow-lg 스타일을 갖는다', async ({ page }) => {
    await page.goto('/');
    const searchWrapper = page.locator('.shadow-lg.border.border-gray-100');
    await expect(searchWrapper.first()).toBeVisible();
  });
});
...
test.describe('카테고리 필터', () => {
  ...
  test('카테고리별 검색 키워드가 올바르다', async ({ page }) => {
    ...
    expect(result.coffee.keyword).toBe('카페');
    expect(result.coffee.code).toBe('CE7');
    expect(result.fuel.keyword).toBe('주유소');
    expect(result.fuel.code).toBe('OL7');
    expect(result.food.keyword).toBe('맛집');
    expect(result.food.code).toBe('FD6');
    expect(result.convenience.keyword).toBe('편의점');
...
test.describe('E2E 경로 검색 흐름', () => {
  test('도착지 검색 → 자동완성 선택 → 경로 표시 흐름', async ({ page }) => {
    ...
    // 4. 자동완성 결과가 있으면 선택 (카카오 API 상태에 따라)
    ...
    if (resultCount > 0) {
      ...
      // 5. 경로 뷰로 전환 시 RoutePanel이 표시되어야 함
      const routePanel = page.locator('.bg-white.rounded-2xl.shadow-lg.p-4');
      const routeExists = await routePanel.isVisible().catch(() => false);

      if (routeExists) {
        // 카테고리 칩 표시 확인
        const allChip = page.locator('button').filter({ hasText: '전체' });
        const chipExists = await allChip.first().isVisible().catch(() => false);
        expect(chipExists).toBe(true);
      }
    }
    // API가 403이면 자동완성 결과 없음 - 정상 동작
  });
...
test.describe('모바일 뷰', () => {
  ...
  test('모바일에서 검색바가 올바르게 표시된다', async ({ page }) => {
    await page.goto('/');
    const searchInput = page.locator('input[placeholder="어디로 갈까요?"]');
    await expect(searchInput).toBeVisible();
